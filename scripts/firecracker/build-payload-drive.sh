...
# carrying EVERYTHING llmbox-specific — the static guest, the standalone
...
# drive at /payload and runs /payload/entrypoint. Swapping in a different guest means
...
# untouched (and cached) across guest changes. Attach both at boot:
...
# Everything the payload carries besides the guest is sourced from the repo
...
# amd64 llmbox-guest built above and the amd64 rootfs the payload rides.
...
# rootfs is guest-agnostic, so the payload entrypoint seeds them into /root at boot.
...
# /payload. It owns all the guest-specific wiring the base deliberately does not
