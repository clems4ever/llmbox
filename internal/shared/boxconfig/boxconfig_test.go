...
func TestBoxLimitsConvertsUnits(t *testing.T) {
	got := BoxLimits(BoxConfig{MemoryMB: 2048, CPUs: 1.5, PidsLimit: 512, MaxBoxes: 7})
	...
	if zero := BoxLimits(BoxConfig{}); zero.MemoryBytes != 0 || zero.NanoCPUs != 0 || zero.PidsLimit != 0 || zero.MaxBoxes != 0 {
