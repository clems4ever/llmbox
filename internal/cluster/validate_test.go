...
func TestValidateCreateImageAllowlist(t *testing.T) {
	...
	if err := p.validateCreate(docker.CreateOptions{BoxID: "b1", Image: "good:2"}); err != nil {
		t.Errorf("allowed image rejected: %v", err)
	}
	if err := p.validateCreate(docker.CreateOptions{BoxID: "b1", Image: "evil:latest"}); err == nil {
...
func TestDispatchRejectsInvalidCreate(t *testing.T) {
	...
	req := func(opts docker.CreateOptions) frame {
		p, _ := encodePayload(createReq{Opts: opts})
		return frame{Type: frameReq, Method: methodCreate, Payload: p}
	}
	if _, err := dispatch(context.Background(), fake, req(docker.CreateOptions{BoxID: "Bad ID"}), policy); err == nil {
		t.Error("invalid box id should be rejected by dispatch")
	}
	// Disallowed image is rejected.
	if _, err := dispatch(context.Background(), fake, req(docker.CreateOptions{BoxID: "b1", Image: "evil"}), policy); err == nil {
	...
	if _, err := dispatch(context.Background(), fake, req(docker.CreateOptions{BoxID: "b1", Image: "good:1"}), policy); err != nil {
