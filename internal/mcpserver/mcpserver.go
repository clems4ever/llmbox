...
func NewServer(b Backend, name, version string) *mcp.Server {
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name: "create_llmbox",
		Description: "Create a new sandboxed box (an 'llmbox'). The box's workload is " +
			"provisioned by its spoke's init script; use exec_llmbox to run commands inside it.",
	}, h.toolCreate)

	mcp.AddTool(srv, &mcp.Tool{
		Name:        "get_llmbox",
		Description: "Look up an llmbox by its box ID (the one given to create_llmbox).",
	}, h.toolGet)

	mcp.AddTool(srv, &mcp.Tool{
		Name:        "list_llmboxes",
		Description: "List the sandboxed boxes managed by this server.",
	}, h.toolList)
...
type createOutput struct {
	BoxID      string `json:"box_id" jsonschema:"the box ID you assigned; pass it to get/destroy/exec to reference this box"`
...
//
// @testcase TestToolCreate calls create_llmbox and checks the box and instance IDs.
...
type getOutput struct {
	...
	InstanceID  string `json:"instance_id,omitempty" jsonschema:"an opaque backend generation token for this incarnation of the box"`
	Description string `json:"description,omitempty" jsonschema:"the description supplied when the box was created, if any"`
}

// toolGet handles the get_llmbox tool: it looks up a box by box ID and returns
// its box ID, instance ID, and description. Use list_llmboxes for a box's live
// state and phase.
//
...
// @return *mcp.CallToolResult Always nil; structured output is returned instead.
// @return getOutput The box's box ID, instance ID, and description.
// @error error if no box ID is given or no box has that box ID.
//
// @testcase TestToolGet returns a box looked up by box ID.
...
type listOutput struct {
	Boxes []api.BoxView `json:"boxes" jsonschema:"the boxes managed by this server"`
}
