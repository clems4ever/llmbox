...
// value (no locks, no pointers into the server) so the tool layer never reaches
// back into the server's internals and tests can construct one directly.
...
// Backend is everything the MCP tool layer needs from the server. The HTTP
// server implements it; tests supply a fake. The OAuth secret is intentionally
// absent: CreateBox returns only a token, and AuthPageURL turns that token into
