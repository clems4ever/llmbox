...
// remote PTY tracks the visible geometry.
...
/** TerminalModal renders a live shell into the selected box.
 *
...
export function TerminalModal({ box, opened, onClose }: TerminalModalProps): JSX.Element {
  ...
  return (
    ...
    >
      {opened && box && <TerminalView boxId={id} onClose={onClose} />}
    </Modal>
  );
}
 ...
 * down on close), keeping no stale socket or buffer between sessions.
 *
 * @arg props The box id to connect to and a close callback for a dead socket.
 * @return JSX.Element The terminal host element and a status line.
 */
function TerminalView({ boxId, onClose }: { boxId: string; onClose: () => void }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      fontSize: 13,
      theme: { background: "#1a1614", foreground: "#f5efe9" },
    });
  ...
  return (
    <div>
      <div ref={hostRef} data-testid="terminal-host" style={{ height: "60vh", width: "100%" }} />
