...
* ~~We want to have a flag to enforce authentication of the user before being able to activate a box. This is so that nobody can register the box before on behalf of who requested the creation.~~ (Done via `auth.google` OIDC sign-in gating activation + domain/email allowlist. Follow-ups below.)
* Authenticate MCP clients per-user so box *creation* is no longer open, and bind a box to the specific initiator (owner-email binding) instead of any allow-listed user.
